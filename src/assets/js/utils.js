...
export const shuffle = function (array) {
  ...
  const shuffledArray = [];
  // tant qu'il y a des éléments dans la copie
  while (arrayCopy.length > 0) {
    // tirer un nombre x entre 0 et la longueur de array
    const randomNumber = getRandomInt(arrayCopy.length);
    // copier l'élément à l'index x du tableau copie dans shuffledArray
    shuffledArray.push(arrayCopy[randomNumber]);
    // retirer l'élément de la copie du tableau
